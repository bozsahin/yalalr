...
;; This version generates IC names as offsets from a base.
;; A Symbol table keeps these offsets and type info. Its format is
;; key: (name blocknon)  value: (type offset)
;;   type is symbol or number.  Each type takes a fixed amount of space (in bytes, say)
;;   offset starts with 0, and increases per length required for type.

(defun make-symtab (size)
  (make-hash-table :test #'equal :size size :rehash-threshold 0.8))  ; we need equal to match a list of values as key
